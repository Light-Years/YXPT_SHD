...
@property (nonatomic,copy) void (^dialogViewCompleteHandle)(HYAlertView *, NSInteger,NSString *);
...
//    titleLabel.font = [UIFont boldSystemFontOfSize:17.0f];
    ...
    for (int i = 0; i < _buttonTitleList.count; i++)
    {
        ...
        if(i > 0 && i == _buttonTitleList.count - 1)//0xe14258
        {
        ...
        {
//            [btn setTitleColor:UIColorFromRGB(0x333333) forState:UIControlStateNormal];
            [btn setTitleColor:[UIColor colorWithRed:37/255.0 green:208/255.0 blue:140/255.0 alpha:1.0] forState:0];
        }
...
-(void)buttonAction:(UIButton *)sender
{
    ...
    UITextField *text = self.textFields[0];

    __weak typeof(*&self) weakSelf = self;

    if(_dialogViewCompleteHandle)
    {
        _dialogViewCompleteHandle(weakSelf, selIndex,text.text);
    }
    [self closeView];
}
...
-(void)showInView:(UIView *)baseView completion:(void (^)(HYAlertView *, NSInteger, NSString *))completeBlock
{
 ...
 *  显示弹出框
 */
-(void)showWithCompletion:(void (^)(HYAlertView *, NSInteger,NSString *))completeBlock
{
